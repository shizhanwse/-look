...
import { Toast,Indicator } from 'bh-mint-ui2';
...
if (process.env.NODE_ENV !== 'production') {
  console.log('devl');
  window.host = 'http://localhost:8080/emap/';
  window.jzgbh = 2010040;
}else{
    console.log('production');
    window.host = WEBPACK_CONIFG_HOST;
    utils.Get(host + api.dqdlrxxcx, {
    }).then(function (res) {
        window.jzgbh = Number(res.data.datas.dqdlrxxcx.rows[0].JZGBH);
        Indicator.close();                   
    }).catch(function(res){
        Indicator.close();
        console.log(res);
